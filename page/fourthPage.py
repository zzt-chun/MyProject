...
class FourthPage():
    ...
    def create_buttons(self):
        ...
        tk.Button(f0, text="获取手机ip", command=self.get_device_ip, width=10).grid(row=0, column=1, rowspan=2, padx=5,
                                                                                pady=10)
        ...
        tk.Button(f0, text="一键截屏并保存到电脑", command=self.save_screen, width=18, height=3).grid(row=0, column=5, rowspan=2,
                                                                                            padx=5, pady=15)
    ...
    def save_screen(self):
        ...
        get_iamge = "adb -s %s:%s pull /sdcard/screenshot.jpg %s\%s.jpg" % (
        self.dev_ip, self.dev_port, self.image_save_path, image_name)
