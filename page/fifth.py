...
class FifthPage(object):
    ...
    def create_buttons(self):
        ...
        tk.Button(lf_2, text="修改创建时间", width=10, command=lambda: self.change_date(ent1)).grid(row=1, column=2, padx=5, pady=5)
        ...
        tk.Button(lf_2, text="修改过滤条件", width=10, command=lambda: self.change_filter(ent2)).grid(row=2, column=2, padx=5, pady=5)
    ...
    def button_check_git(self):
        ...
        logs_target = self._project.commits.list(ref_name=self.target_branche, all=True, since=self.target_branche_create_time)
        logs_master = self._project.commits.list(ref_name=self._master_name, all=True, since=self.target_branche_create_time)
