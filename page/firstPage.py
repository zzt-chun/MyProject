...
class FirstPage():
    ...
    def creat_buttons(self):
        ...
        tk.Button(self.tab1, text='导入', command=lambda: self.import_file_1(self.entry_1, com1, com3, 0)).grid(row=0,
                                                                                                              column=2,
                                                                                                              padx=5,
                                                                                                              pady=1)
        tk.Button(self.tab1, text='导入', command=lambda: self.import_file_1(self.entry_2, com2, com4, 1)).grid(row=1,
                                                                                                              column=2,
                                                                                                              padx=5,
                                                                                                              pady=1)
        ...
        tk.Button(self.tab1, text='开始对比', command=self.compare_excel, height=3, width=15).grid(row=0, column=5,
                                                                                               rowspan=2, padx=6,
                                                                                               pady=1)
        # 第二页
        ...
        tk.Button(self.tab2, text='导入', command=lambda: self.import_file_2(self.entry_1_2, 0)).grid(row=0, column=2,
                                                                                                    padx=5, pady=1)
        tk.Button(self.tab2, text='导入', command=lambda: self.import_file_2(self.entry_2_2, 1)).grid(row=1, column=2,
                                                                                                    padx=5, pady=1)
        ...
        tk.Button(self.tab2, text='提取版本号', width=13, command=self.get_versions_button).grid(row=0, column=6, padx=4,
                                                                                            pady=1, rowspan=2)
        tk.Label(self.tab2, text='--》').grid(row=0, column=7, padx=4, pady=1, rowspan=2)
        tk.Button(self.tab2, text='开始检查', width=13, command=self.check_version).grid(row=0, column=8, padx=4, pady=1,
                                                                                     rowspan=2)
        ...
        tk.Button(self.parent, text='保存日志', command=lambda: self.save_log(self.tex), width=15).pack(anchor=tk.S,
                                                                                                    side=tk.RIGHT,
                                                                                                    padx=3, pady=5)
        tk.Button(self.parent, text='清除日志', command=lambda: self.clear_log(self.tex)).pack(anchor=tk.S, side=tk.RIGHT,
                                                                                           padx=3, pady=5)
    ...
    def check_version(self):
        ...
        for name in data_key_1.keys():
            if name in data_key_2.keys():
                value_1 = float(data_2[t_version_rule[3]][data_key_2[name]]) - float(
                    data_1[t_version_rule[3]][data_key_1[name]])
                value_2 = float(data_2[t_version_rule[4]][data_key_2[name]]) - float(
                    data_1[t_version_rule[4]][data_key_1[name]])
    ...
    def get_versions_button(self):
        ...
        e = tk.Text(top)
        e.pack(expand=True)
        tk.Button(top, text='提取篮球version内容', command=lambda: self.get_version(e.get(1.0, tk.END), 0)).pack(
            side=tk.RIGHT)
        tk.Button(top, text='提取老足球version内容', command=lambda: self.get_version(e.get(1.0, tk.END), 1)).pack(
            side=tk.RIGHT)
    ...
    def import_file_2(self, e, index):
        ...
        for i in range(2, len(t_version_rule)):
            if t_version_rule[i] not in rows_1 or t_version_rule[i] not in rows_2:
                self.tex.insert(tk.END,
                                self.get_now_time() + ': 发现导入文件中存在不包含<%s>字段的情况，请检查是否为合法的t_version文件\n' % t_version_rule[
                                    i])
                return
        ...
        if self.key_2[0] != self.key_2[1]:
            self.tex.insert(tk.END,
                            self.get_now_time() + ': 表一中区服列表名为<%s>， 而表二中的区服列表名为<%s>， 请检查两份表是否为相同项目的t_version文件\n' % (
    ...
    def compare_excel(self):
        ...
        if None in self.key:
            ...
            if len(data1) == 0 or len(data2) == 0:
                ...
                return
            show_dif_ui(dif_array, dif_row_excel, dif_column_excel, dif_row_data, dif_column_data, self.tex,
                        ' and '.join([os.path.basename(self.path[0]), os.path.basename(self.path[1])]),
                        [data1[0], data2[0]])
        else:
        ...
        show_dif(dif_array, dif_data, dif_excel, self.tex,
                 ' and '.join([os.path.basename(self.path[0]), os.path.basename(self.path[1])]))
